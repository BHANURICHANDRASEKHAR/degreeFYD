...
function dataisValid(data)
{
   ...
   if(data.firstname.trim().length==0 ||data.lastname.trim().length==0|| data.password.trim().length==0 || data.confirmPassword.trim().length==0 || data.email.trim().length==0 || data.state.trim().length==0   )
   {
